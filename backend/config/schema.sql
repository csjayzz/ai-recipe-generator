...
--jsonb (Binary JSON) is a PostgreSQL data type that stores JSON data in a decomposed binary format
,
...
fiber DECIMAL(10, 2),
created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
--the unique constraint ensures that each recipe has only one nutrition entry, preventing duplicate entries for the same recipe and maintaining data integrity.
UNIQUE (recipe_id)
);
